...
本仓库为论文《小模型多段后训练梯度失衡诊断与调度干预》（前作：《反问式自博弈微调：让垂直领域语言模型通过反问学会更好地回答》）的官方代码与数据实现，提供完整的可复现实验流程，含梯度失衡诊断工具链与 CQLS 调度干预（见 [梯度诊断与调度干预](#梯度诊断与调度干预新增)）。

📄 论文：《小模型多段后训练梯度失衡诊断与调度干预》（北大核心期刊《计算机应用》，审稿中）
